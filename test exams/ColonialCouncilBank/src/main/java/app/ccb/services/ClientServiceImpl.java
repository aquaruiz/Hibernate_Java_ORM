...
public class ClientServiceImpl implements ClientService {
    ...
    public String exportFamilyGuy() {
        Client client = clientRepository.findClientByBankCars().get(0);
        StringBuilder stringBuilder = new StringBuilder();

        stringBuilder.append(String.format("Full Name: %s %nAge: %d %nBank Account: %s%n",
            client.getFullName(),
            client.getAge(),
            client.getBankAccount().getAccountNumber()));

        client.getBankAccount().getCards().forEach(
            c -> {
                stringBuilder.append(String
                    .format("    Card Number : %s %n    Card status: %s %n",
                    c.getCardNumber(),
                    c.getCardStatus()));
            }
        );
